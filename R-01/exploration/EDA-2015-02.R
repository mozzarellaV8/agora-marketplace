...
length(unique(p15$product)) # 61625
length(unique(p15$vendor))  # 1960
length(unique(p15$cat))     # 15

# aggregate vendors -----------------------------------------------------------

v15 <- p15
v15$subcat <- as.character(v15$subcat)
v15$subsubcat <- as.character(v15$subsubcat)
v15$subcat[is.na(v15$subcat)] <- "-"
v15$subsubcat[is.na(v15$subsubcat)] <- "-"

# this might be it.
v15$allcat <- paste(v15$cat, v15$subcat, v15$subsubcat, sep = ", ")
v15$allcat <- gsub("\\b,\\s-$", "", v15$allcat)
v15$allcat <- gsub("\\b,\\s-,\\s-$", "", v15$allcat)

levels(as.factor(v15$allcat))
v15$allcat <- factor(v15$allcat) # 101 levels

v15$from <- as.character(v15$from)
v15$from[is.na(v15$from)] <- "No Info"
v15$from <- factor(v15$from)

levels(v15$from)    # 66
levels(v15$vendor)  # 1960

v15 <- subset(v15, select = c("date", "vendor", "product", "usd", "rate", "cat", 
                              "subcat", "subsubcat", "allcat",
                              "from", "to", "month", "fb", "feedback"))

length(unique(v15$product)) # 61625

write.csv(vsdf, file = "~/GitHub/agora-data/wf-p-location.csv", row.names = F)
write.csv(v15, file = "~/GitHub/agora-data/v15-arules-00.csv", row.names = F)
