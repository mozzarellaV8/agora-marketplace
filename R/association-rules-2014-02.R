...
# how much --------------------------------------------------------------------
par(mar = c(7, 11, 8, 7), las = 1,  bty = "l", family = "HersheySans")
plot(p14$date, p14$usd, ylab = "$", col = "#00000090",
     main = "Agora Marketplace 2014: Product Prices < $750k")

plot(p14$date, log2(p14$usd), ylab = "log2(usd)", col = "#00000090",
     main = "Agora Marketplace 2014: Product Prices < $750k (log base 2 USD)")

kde <- density(p14$usd, kernel = "gaussian")
plot(kde)

# where and what? -------------------------------------------------------------
par(mar = c(10, 22, 4, 6), las = 2,  bty = "l", family = "HersheySans")
spineplot(p14$cat, p14$from, main = "plot(p14$cat, p14$from)",
          xlab = "", ylab = "", subset(p14, p14$usd <= 1000))
spineplot(p14b$subcat, p14b$from, main = "(p14b$subcat, p14b$from)",
          xlab = "", ylab = "")
spineplot(p14b$subsubcat, p14b$from, main = "plot(p14b$subsubcat, p14b$from)",
          xlab = "", ylab = "")


# 1000 USD subset -------------------------------------------------------------
# curious - at what USD max does the data distribute normally?

# subset list under $1000 with/without feedback.
p14b <- subset(p14, p14$usd <= 1000) # 905805 

summary(p14b$usd)
#     Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
#    0.00   24.58   75.23  162.00  209.40 1000.00

par(mar = c(7, 11, 8, 7), las = 1,  bty = "l", family = "HersheySans")
plot(p14b$date, p14b$usd, ylab = "$", col = "#00000090",
     main = "Agora Marketplace 2014: Product Prices < $1,000")

# Feedbacks ONLY --------------------------------------------------------------
p14$feedback <- as.character(p14$feedback)

# list under 1000$ USD and contains feedback.
...
par(mar = c(10, 16, 4, 6), las = 2,  bty = "l", family = "HersheySans")
spineplot(p14$cat, p14$from, main = "plot(p14c$cat, p14c$from)",
          xlab = "", ylab = "", ylevels = seq(69, 0, -1))

spineplot(p14$from, p14$cat, main = "plot(p14c$from, p14c$cat)",
          xlab = "", ylab = "", ylevels = seq(69, 0, -1))

spineplot(p14$subcat, p14$from, main = "plot(p14$subcat, p14$from)",
          xlab = "", ylab = "", ylevels = seq(69, 0, -1),
          xaxlabels = sort(levels(p14$subcat), decreasing = T),
          yaxlabels = sort(levels(p14$from), decreasing = T))

spineplot(p14$from, p14$subcat, main = "plot(p14$from, p14$subcat)",
          xlab = "", ylab = "", ylevels = seq(69, 0, -3),
          xaxlabels = sort(levels(p14$from), decreasing = T),
          yaxlabels = sort(levels(p14$subcat), decreasing = F))

spineplot(p14$subsubcat, p14$from, main = "plot(p14$subsubcat, p14$from)",
          xlab = "", ylab = "", ylevels = seq(69, 0, -1),
          xaxlabels = sort(levels(p14$subsubcat), decreasing = T),
          yaxlabels = sort(levels(p14$from), decreasing = T))

spineplot(p14$from, p14$subsubcat, main = "plot(p14$subsubcat, p14$from)",
          xlab = "", ylab = "", ylevels = seq(69, 0, -1),
          yaxlabels = sort(levels(p14$subsubcat), decreasing = T))

levels(p14$subsubcat)

ft <- as.data.frame(table(p14$from))
colnames(ft) <- c("location", "count")


# reduce variables for better mosaic plots
mosaic(count~location, data = ft)
mosaic(location~count, data = ft)
